...
### 📱 Premium Offline Immunization Record & Vaccination Tracker
...
**Fully Offline Mobile & Tablet Immunization Log App** 🚀
 
*A modern, offline-first immunization record and vaccination tracker built with Ionic React and Capacitor. Keep track of your family's vaccination records, upcoming doses, and medical history — all without an internet connection. Supports IPFS-based decentralized backups for tamper-proof records.*
...
The user interface is optimized for rapid vaccination records update, touch interactions, and offline-first storage:
...
All vaccination records, schedules, and preferences are stored securely on the device's local storage. The app requires zero internet connectivity to function.

### 🔒 Patient Data Privacy
No cloud connections, telemetry, or remote databases. Sensitive medical history and vaccine logs remain strictly on the user's device.

### 📋 Record Management
Create and manage multiple immunization sheets. Search, duplicate, edit, and delete vaccination profiles from a centralized file registry.

</td>
<td width="50%">

### 💼 SocialCalc Spreadsheet Engine
Built on a touch-optimized wrapper over the robust **SocialCalc spreadsheet engine**. Supports grid editing, column resizing, custom cell styling, alignment, and formula calculations for tracking vaccine dates.

### 🧾 Multi-Sheet Templates
Pre-designed immunization record layouts optimized for various viewports:
- **Mobile**: Vaccination record sheet optimized for mobile layouts.
- **Tablet**: Comprehensive vaccination record sheet.
- **Footer Tabs**: Categorized logs (e.g., Record 1, Record 2, Record 3, Record 4 sheets).
...
### ✏️ Rapid Form Entry
Fill out clinical records and log entries through a clean, intuitive form interface that maps directly onto the spreadsheet grid.

</td>
<td width="50%">

### 📄 Export as PDF & CSV
Generate professional, print-ready summaries as high-resolution PDFs or export raw data as CSV.
...
Pin and backup immunization files to the IPFS network (via Pinata Gateway) to create immutable, tamper-proof logs that are verifiable and retrievable.
...
| **Language** | **TypeScript** | Strict type safety across log data models, templates, and service layers |
| **Native Integration** | **Capacitor 8** | Runtime bridge for native iOS and Android compilation |
| **Spreadsheet Engine** | **SocialCalc (Legacy)** | Formula evaluation, grid rendering, multi-sheet log state management |
| **Storage** | **localStorage** | Offline JSON serialization for logs, templates, and preferences |
| **Build & Bundle** | **Vite 7** | Fast HMR development server and optimized production bundling |
| **PDF Generation** | **jsPDF + html2canvas** | Client-side high-fidelity PDF export of immunization logs |
...
┌────────────────────────────────────────────────────────┐
│                   💉 IONIC UI LAYER                    │
│   Dashboard Home │ Log Editor │ Files Registry         │
└───────────────────────────┬────────────────────────────┘
                            │
┌───────────────────────────▼────────────────────────────┐
│                  🔄 REACT STATE BRIDGE                 │
...
- **State Sync**: Context loads persisted logs from `localStorage` on initial mount.
...
- **IPFS Backup**: Logs can be pinned to the IPFS network via Pinata for decentralized, tamper-proof archival.
