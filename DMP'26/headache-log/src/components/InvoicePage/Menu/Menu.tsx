...
const Menu: React.FC<{
  ...
  const doPrint = async () => {
    ...
    if (isPlatform("hybrid") && isPlatform("android")) {
      try {
        ...
        await Printer.print({
          content: htmlContent,
          name: selectedFile || "Headache Log",
      ...
      const printWindow = window.open("", "Print Headache Log");
      if (printWindow) {
  ...
  const doGeneratePDF = async (filename?: string) => {
    try {
      ...
      const pdfFilename = filename || selectedFile || "Headache Log";
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                await Share.share({
                  ...
                  text: "Headache Log PDF generated successfully",
                  url: tempFile.uri,
  ...
  const doGenerateCSV = async (filename?: string) => {
    try {
      ...
      const csvFilename = filename || selectedFile || "Headache Log";
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                await Share.share({
                  ...
                  text: "Headache Log data exported as CSV",
                  url: tempFile.uri,
  ...
  const sendEmail = async () => {
    if (isPlatform("hybrid")) {
      try {
        const content = AppGeneral.getCurrentHTMLContent();
        const invoiceIdentifier = invoiceId || selectedFile || "Headache Log";

        // For iOS, try using Share API with PDF as it's more reliable
        if (isPlatform("ios")) {
          try {
            // Generate PDF for sharing via email on iOS
            setToastMessage("Preparing headache log for email...");
            setShowToast1(true);
              ...
              reader.onloadend = async () => {
                ...
                try {
                  ...
                  await Share.share({
                    ...
                    text: `Please find the attached headache log with ID: ${invoiceIdentifier}`,
                    url: tempFile.uri,
                    dialogTitle: "Share Headache Log via Email",
                  });
                  ...
                  setToastMessage("Failed to share headache log. Please try again.");
                  setShowToast1(true);
                }
              };
              reader.readAsDataURL(pdfBlob as Blob);
          ...
          await EmailComposer.open({
            ...
            cc: [],
            bcc: [],
            body: `Please find the attached headache log with ID: ${invoiceIdentifier}`,
