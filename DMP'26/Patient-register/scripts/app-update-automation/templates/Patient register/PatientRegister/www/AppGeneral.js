...
function showPrintDialog(){
    ...
    cordova.plugins.printer.print(html, 'QuoteInvoice.html', function () {
...
function exportAsCsv(){
    ...
    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function(fileSystem) {
                             ...
                             //alert("fs");
                             fileSystem.root.getFile("QuoteInvoice.csv", {create: true}, function(entry) {
                                                     var fileEntry = entry;
                                                     console.log(entry);
...
item.init=function(){
    ...
    $.ajax({
           ...
           var productList=[];
           productList.push({"Feature": "10Pdf","Id": "2014qi10Pdf","Purchase":"No","Consumed":0,"Own":0 });
           productList.push({"Feature": "25Pdf","Id": "2014qi25Pdf","Purchase":"No","Consumed":0,"Own":0 });
           productList.push({"Feature": "50Pdf","Id": "2014qi50Pdf","Purchase":"No","Consumed":0,"Own":0 });
           productList.push({"Feature": "100Pdf","Id": "2014qi100Pdf","Purchase":"No","Consumed":0,"Own":0});
           productList.push({"Feature": "fb-tw-sms-whatsapp","Id": "2015qi10share","Purchase":"No","Consumed":0,"Own":0});
           productList.push({"Feature": "email-print-save","Id": "2016qiSavePrintEmail","Purchase":"Yes","Consumed":0,"Own":1});
           productList.push({"Feature": "pdf-ibooks","Id": "2016qiSavePdf","Purchase":"No","Consumed":0,"Own":0});
...
var IAP = {
list: [ "2014qi10Pdf", "2014qi25Pdf", "2014qi50Pdf","2014qi100Pdf","2015qi10share" ,"2015qiCloud" , "2016qiSavePrintEmail" ,"2016qiSavePdf"]
};
...
item.getAllProducts=function(){
    ...
    var renderIAPs = function (el) {
        if (IAP.loaded) {
            //alert(IAP.loaded);
            var coins10  = IAP.products["2014qi10Pdf"];
            var coins100 = IAP.products["2014qi25Pdf"];
            var coins1000 = IAP.products["2014qi50Pdf"];
            var coins10000 = IAP.products["2014qi100Pdf"];
...
item.successCallback=function(productId){
    ...
    switch(productId){
        ...
        case "2016qiSavePrintEmail":
            ...
            products[5].Own=parseInt(25)+parseInt(left);
...
function cloudInappSuccess(){
    ...
    $.ajax({
           ...
           data: {action:"purchase",appname:"Quote Invoice"},
           dataType: 'json',
           success: function(response) {
...
function setCloudRestoreItems(products){
    ...
    $.ajax({
           type: 'POST',
           url: 'http://aspiringapps.com/restore',
           data: {content:products,action:"inapp",appname:"Quote Invoice"},
           dataType: 'json',
           success: function(response) {
           ...
           window.setTimeout(function(){
                ...
                // showToast("Please Login to access Quote Invoice on web");
...
function updateShareItem(){
    ...
    var itemConsumed;
    for(i in products){
        if(products[i].Id == "2015qi10share")
            itemConsumed = i;
    }
...
function checkBeforeSave(file){
    ...
    if(products[0].Purchase == "Yes"){
        ...
        navigator.notification.alert("Save as exhausted.Kindly purchase by clicking on the Purchase Tab",null,"Sync and backup to server");
...
function saveToWeb(file){
    ...
    var interval = setInterval(function(){
                               ...
                               request(messages, function(result){
                                       ...
                                       message.type="POST";
                                       message.format = 'json';
                                       message.data={action:"savefile",fname:fname,data:val1,appname:"Quote Invoice"};
...
function updateCloudInapp(){
    ...
    message.type="POST";
    message.format = 'json';
    message.data={action:"update",appname:"Quote Invoice"};
...
function showFeedback(){
    cordova.require('emailcomposer.EmailComposer').show({
                                                        ...
                                                        cc: '',
                                                        bcc: '',
                                                        subject: 'Quote Invoice: Please share your feedback',
                                                        body: '',
                                                        isHtml: true,
                                                        onSuccess: function (winParam) {
                                                        console.log('EmailComposer onSuccess - return code ' + winParam.toString());
...
function refer(){
    ...
    var onConfirm = function(buttonIndex){
        if(buttonIndex == 4) return;
            ...
            window.plugins.socialsharing.shareViaFacebook('Keep a track of your exercise routine using Quote Invoice', 'www/img/bi.png' /* img */, 'https://itunes.apple.com/us/app/fitness-planner/id600477433?ls=1&mt=8' /* url */, function() {
                                                          console.log('share ok');
                                                          },
                                                          function(errormsg){console.log(errormsg)});
            ...
            window.plugins.socialsharing.shareViaTwitter('Keep a track of your exercise routine using Quote Invoice','www/img/bi.png','https://itunes.apple.com/us/app/fitness-planner/id600477433?ls=1&mt=8');
        }
            ...
            cordova.require('emailcomposer.EmailComposer').show({
                                                                ...
                                                                bcc: '',
                                                                subject: 'Quote Invoice',
                                                                body: 'Keep a track of your exercise routine using Quote Invoice \n https://itunes.apple.com/us/app/fitness-planner/id600477433?ls=1&mt=8',
                                                                isHtml: true,
                                                                onSuccess: function (winParam) {
                                                                console.log('EmailComposer onSuccess - return code ' + winParam.toString());
                                                                },
    ...
    navigator.notification.confirm(
                                   'Refer Quote Invoice via', // message
                                   onConfirm,  // callback to invoke with index of button pressed
...
function showSignaturePrompt(){
    ...
    if(currsheet == "sheet2" || currsheet == "sheet3"){
        
        
    }
    else{
        showToast("Signature can be added only in Invoice1 and Invoice2 sheets");
        return;
    ...
    var onConfirmLogo = function(buttonIndex){
        if(buttonIndex == 3)return;
        else if(buttonIndex==2){
            signatureRemove();
        }
        else if(buttonIndex==1){
            showAddSignaturePrompt();
        }
        
    }
    
    navigator.notification.confirm(
                                   'Add or remove Signature',  // message
                                   onConfirmLogo,              // callback to invoke with index of button pressed
                                   'Signature',            // title
                                   ['Add Signature','Remove Signature','Cancel']          // buttonLabels
                                   );
...
function showAddSignaturePrompt(){
    var control = SocialCalc.GetCurrentWorkBookControl();
    var currsheet = control.currentSheetButton.id;
    
    
    
    var onConfirmAdd = function(buttonIndex){
        if(buttonIndex == 3)return;
        else if(buttonIndex==2){
            //logoRemove();
            addSignatureUrl(2);
    ...
    navigator.notification.confirm(
                                   'Complete action using',  // message
                                   onConfirmAdd,              // callback to invoke with index of button pressed
                                   'Add Logo',            // title
                                   ['Camera','Photos','Cancel']          // buttonLabels
                                   );
    
    
}

function addSignatureUrl(val){
    
    if(val==1){
        navigator.camera.getPicture(onSuccess, onFail, { quality: 50, destinationType: Camera.DestinationType.DATA_URL, sourceType : Camera.PictureSourceType.CAMERA});
        
        
        function onSuccess(data) {
            ActivityIndicator.show("Adding Signature..");
            ...
            $.ajax({
                   type: 'POST',
                   url: "http://aspiringapps.com/iconimg",
                   data: {content:content , suffix:"jpeg"},
                   dataType: 'json',
                   success: function(data) {
...
function addSignature(link){
    ...
    if(devicefind=="iPad" || devicefind=="default"){
        ...
        var currsheet = control.currentSheetButton.id;
        if(currsheet == "sheet2" || currsheet == "sheet3"){
            var cmd = 'set C31 text t <img src="'+link+'" height="100" width="150"></img>'+"\n";
...
function savePDF(){
    ...
    window.html2pdf.create(
                           ""+content+"",
                           "~/Documents/QuoteInvoice.pdf", // on iOS,
...
document.addEventListener("deviceready", function(){
                          ...
                          var products = JSON.parse(window.localStorage.getItem("inapplocal"));
                          var saveEmail; var savePdf;
                          for(i in products){
                          if(products[i].Id == "2016qiSavePrintEmail"){
                          console.log("saveEmail present");
                          saveEmail=true;
                          }
                          if(products[i].Id == "2016qiSavePdf"){
                          console.log("2016qiSavePdf present");
                          savePdf=true;
                          }
                          ...
                          if(!saveEmail){
                          products.push({"Feature": "email-print-save","Id": "2016qiSavePrintEmail","Purchase":"Yes","Consumed":0,"Own":1});
                          ...
                          if(!savePdf){
                          products.push({"Feature": "pdf-ibooks","Id": "2016qiSavePdf","Purchase":"No","Consumed":0,"Own":0});
                          console.log("new "+JSON.stringify(products));
                          window.localStorage.setItem("inapplocal",  JSON.stringify(products));
                          setCloudRestoreItems(JSON.stringify(products));
                          }
...
function updateCounter(){
    ...
    if(left <= 15){
        navigator.notification.alert("You have limited number of times remaining for doing Save as ,Print and Email.Kindly buy from Purchase tab",null,"25 times Save as,Print and Email");
    }
    //alert(pdfLeft);
    window.localStorage.setItem("inapplocal",JSON.stringify(products));
    //alert("4");
...
function checkBeforeUse(feature,name){
    ...
    if(products[itemAvailable].Consumed <= products[itemAvailable].Own && products[itemAvailable].Purchase == "Yes"){
        //exportPDF();
        //return true;
        //alert("can use "+feature);
        if(feature == "email"){
            ...
            cordova.require('emailcomposer.EmailComposer').show({
                                                                ...
                                                                cc: '',
                                                                bcc: '',
                                                                subject: 'Quote Invoice Workbook',
                                                                body: content,
                                                                isHtml: true,
                                                                attachments: [{
                                                                              mimeType: 'text/html',
        ...
        //alert("no");
        navigator.notification.alert("You have exceeded 25 times of using Email, Print and Save as.To continue using the feature, please click on Purchase Tab and purchase",null,"Email, Print and Save as");
        return;
    }
