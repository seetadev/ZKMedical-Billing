...
txt = re.sub(
    r'(Default Currency</h4>\s*\n?\s*<p\s+className="settings-card-desc">)[^<]*(</p>)',
...
for tpl_file in ["mobile.json", "tablet.json"]:
    ...
    key = "mobile" if tpl_file == "mobile.json" else "tablet"
    names = data["templates"]["footerTabNames"].get(key, [])
    
    if "footers" in tpl:
        for footer in tpl["footers"]:
            idx = footer["index"]
            if idx - 1 < len(names):
                footer["name"] = names[idx - 1]
