...
const Home: React.FC = () => {
  ...
  // Check if current sheet supports graphs
  const isGraphAvailable = () => {
    if (!activeTemplateData || !currentSheetId) return false;
    return activeTemplateData.showGraph?.[currentSheetId] === true;
  };

  // Get current sheet data for visualization
  const getCurrentSheetData = () => {
    try {
      ...
      const currentSheet = control.workbook.spreadsheet.sheet;
      return currentSheet;
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <GraphVisualization
          isOpen={showGraphVisualization}
          onClose={() => setShowGraphVisualization(false)}
        ...
        <IonFab vertical="bottom" horizontal="end" slot="fixed">
          {/* Visualization Button - Only show if graph is available */}
          {isGraphAvailable() && (
            <IonFabButton
