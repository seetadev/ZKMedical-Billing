...
import { TemplateData, DATA } from "../templates-data";
import { isMSCDataValid } from "../firebase/templateService";
...
export const InvoiceProvider: React.FC<InvoiceProviderProps> = ({
    ...
    try {
      ...
      if (savedActiveTemplateId) {
        ...
        if (templateData) {
          ...
          if (!savedCurrentSheetId && isMSCDataValid(templateData.msc)) {
  ...
  const updateActiveTemplateData = (templateData: TemplateData | null) => {
    ...
    if (templateData && isMSCDataValid(templateData.msc)) {
      setCurrentSheetId(templateData.msc.currentid);
    }
  };
