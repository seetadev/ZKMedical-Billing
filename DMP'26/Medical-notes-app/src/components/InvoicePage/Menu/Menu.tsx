...
const Menu: React.FC<{
  ...
  const doPrint = async () => {
    ...
    if (isPlatform("hybrid") && isPlatform("android")) {
      try {
        ...
        await Printer.print({
          content: htmlContent,
          name: selectedFile || "Medical Notes",
      ...
      const printWindow = window.open("", "Print Medical Notes");
      if (printWindow) {
  ...
  const doGeneratePDF = async (filename?: string) => {
    try {
      ...
      const pdfFilename = filename || selectedFile || "Medical Notes";
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                await Share.share({
                  ...
                  text: "Medical Notes PDF generated successfully",
                  url: tempFile.uri,
  ...
  const doGenerateCSV = async (filename?: string) => {
    try {
      ...
      const csvFilename = filename || selectedFile || "Medical Notes";
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                await Share.share({
                  ...
                  text: "Medical Notes data exported as CSV",
                  url: tempFile.uri,
  ...
  const sendEmail = async () => {
    if (isPlatform("hybrid")) {
      try {
        const content = AppGeneral.getCurrentHTMLContent();
        const invoiceIdentifier = invoiceId || selectedFile || "Medical Notes";

        // For iOS, try using Share API with PDF as it's more reliable
        if (isPlatform("ios")) {
          try {
            // Generate PDF for sharing via email on iOS
            setToastMessage("Preparing medical notes for email...");
            setShowToast1(true);
              ...
              reader.onloadend = async () => {
                ...
                try {
                  ...
                  await Share.share({
                    ...
                    text: `Please find the attached medical notes with ID: ${invoiceIdentifier}`,
                    url: tempFile.uri,
                    dialogTitle: "Share Medical Notes via Email",
                  });
                  ...
                  setToastMessage("Failed to share medical notes. Please try again.");
                  setShowToast1(true);
                }
              };
              reader.readAsDataURL(pdfBlob as Blob);
          ...
          await EmailComposer.open({
            ...
            cc: [],
            bcc: [],
            body: `Please find the attached medical notes with ID: ${invoiceIdentifier}`,
