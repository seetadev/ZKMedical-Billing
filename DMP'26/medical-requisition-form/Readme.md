...
### 📱 Premium Offline Medical Requisition Form & Laboratory Request Tracker
...
**Fully Offline Mobile & Tablet Medical Requisition Form App** 🚀
 
*A modern, offline-first medical requisition form and laboratory request tracker built with Ionic React and Capacitor. Manage laboratory requisitions, patient details, specimen records, and clinical notes — all without an internet connection. Supports IPFS-based decentralized backups for tamper-proof records.*
...
The user interface is optimized for rapid clinical checks, laboratory testing data entry, touch interactions, and offline-first storage:
...
All lab requisitions, specimen logs, clinical notes, and patient data are stored securely on the device's local storage. The app requires zero internet connectivity to function.

### 🔒 Patient Data Privacy
No cloud connections, telemetry, or remote databases. Sensitive medical records and lab requisition sheets remain strictly on the user's device.

### 📋 Record Management
Create and manage multiple requisition sheets. Search, duplicate, edit, and delete requisitions from a centralized file registry.

</td>
<td width="50%">

### 💼 SocialCalc Spreadsheet Engine
Built on a touch-optimized wrapper over the robust **SocialCalc spreadsheet engine**. Supports grid editing, column resizing, custom cell styling, alignment, and formula calculations for lab tracking.

### 🧾 Multi-Sheet Templates
Pre-designed medical requisition layouts optimized for various viewports:
- **Mobile**: Laboratory requisition form sheets optimized for mobile layouts.
- **Tablet**: Comprehensive laboratory requisition form sheets.
- **Footer Tabs**: Categorized forms (e.g., Form 1, Form 2 sheets).
...
### ✏️ Rapid Form Entry
Fill out laboratory details, specimen records, and clinical logs through a clean, intuitive form interface that maps directly onto the spreadsheet grid.

</td>
<td width="50%">

### 📄 Export as PDF & CSV
Generate professional, print-ready reports as high-resolution PDFs or export raw data as CSV.
...
Pin and backup medical requisition files to the IPFS network (via Pinata Gateway) to create immutable, tamper-proof logs that are verifiable and retrievable.
...
| **Language** | **TypeScript** | Strict type safety across form data models, templates, and service layers |
| **Native Integration** | **Capacitor 8** | Runtime bridge for native iOS and Android compilation |
| **Spreadsheet Engine** | **SocialCalc (Legacy)** | Formula evaluation, grid rendering, multi-sheet form state management |
| **Storage** | **localStorage** | Offline JSON serialization for forms, templates, and preferences |
| **Build & Bundle** | **Vite 7** | Fast HMR development server and optimized production bundling |
| **PDF Generation** | **jsPDF + html2canvas** | Client-side high-fidelity PDF export of medical requisition forms |
...
┌────────────────────────────────────────────────────────┐
│                   🔬 IONIC UI LAYER                    │
│   Dashboard Home │ Form Editor │ Files Registry        │
└───────────────────────────┬────────────────────────────┘
                            │
┌───────────────────────────▼────────────────────────────┐
│                  🔄 REACT STATE BRIDGE                 │
...
- **State Sync**: Context loads persisted forms from `localStorage` on initial mount.
...
- **IPFS Backup**: Forms can be pinned to the IPFS network via Pinata for decentralized, tamper-proof archival.
