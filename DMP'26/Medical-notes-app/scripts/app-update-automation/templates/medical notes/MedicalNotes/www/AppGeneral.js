...
function showPrintDialog(){
    ...
    cordova.plugins.printer.print(html, 'MedicalNotes.html', function () {
...
function exportAsCsv(){
    ...
    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function(fileSystem) {
                             ...
                             //alert("fs");
                             fileSystem.root.getFile("MedicalNotes.csv", {create: true}, function(entry) {
                                                     var fileEntry = entry;
                                                     console.log(entry);
...
function cloudInappSuccess(){
    ...
    $.ajax({
           ...
           data: {action:"purchase",appname:"Medical Notes"},
           dataType: 'json',
           success: function(response) {
...
function setCloudRestoreItems(products){
    ...
    /*$.ajax({
           type: 'POST',
           url: 'http://aspiringapps.com/restore',
           data: {content:products,action:"inapp",appname:"Medical Notes"},
           dataType: 'json',
           success: function(response) {
           ...
           //showToast("Please Login to access Medical Notes on web");
...
function checkBeforeSave(file){
    ...
    if(products[0].Purchase == "Yes"){
        ...
        navigator.notification.alert("Save as exhausted.Kindly purchase by clicking on the Purchase Tab",null,"Medical Notes Widget");
...
function saveToWeb(file){
    ...
    var interval = setInterval(function(){
                               ...
                               request(messages, function(result){
                                       ...
                                       message.type="POST";
                                       message.format = 'json';
                                       message.data={action:"savefile",fname:fname,data:val1,appname:"Medical Notes"};
...
function updateCloudInapp(){
    ...
    message.type="POST";
    message.format = 'json';
    message.data={action:"update",appname:"Medical Notes"};
...
function showFeedback(){
    cordova.require('emailcomposer.EmailComposer').show({
                                                        ...
                                                        cc: '',
                                                        bcc: '',
                                                        subject: 'Medical Notes: Please share your feedback',
                                                        body: '',
                                                        isHtml: true,
                                                        onSuccess: function (winParam) {
                                                        console.log('EmailComposer onSuccess - return code ' + winParam.toString());
...
function refer(){
    ...
    var onConfirm = function(buttonIndex){
        if(buttonIndex == 4) return;
            ...
            window.plugins.socialsharing.shareViaFacebook('Medical Notes 2016 using Medical Notes', 'www/img/bi.png' /* img */, 'https://itunes.apple.com/us/app/medication-log/id894930232?ls=1&mt=8#' /* url */, function() {
                                                          console.log('share ok');
                                                          },
                                                          function(errormsg){console.log(errormsg)});
            ...
            window.plugins.socialsharing.shareViaTwitter('Medical Notes 2016 using Medical Notes','www/img/bi.png','https://itunes.apple.com/us/app/medication-log/id894930232?ls=1&mt=8#');
        }
    ...
    navigator.notification.confirm(
                                   'Refer Medical Notes via', // message
                                   onConfirm,  // callback to invoke with index of button pressed
...
function checkBeforeUse(feature,name){
    ...
    if(feature == "email"){
        ...
        cordova.require('emailcomposer.EmailComposer').show({
                                                            ...
                                                            cc: '',
                                                            bcc: '',
                                                            subject: 'Medical Notes Workbook',
                                                            body: content,
                                                            isHtml: true,
                                                            attachments: [{
                                                                          mimeType: 'text/html',
