...
const APP_NAME = "Medical Requisition Form";
...
const Menu: React.FC<{
  ...
  const doPrint = async () => {
    ...
    if (isPlatform("hybrid") && isPlatform("android")) {
      try {
        ...
        await Printer.print({
          content: htmlContent,
          name: selectedFile || "Medical Requisition Form",
      ...
      const printWindow = window.open("", "Print Medical Requisition Form");
      if (printWindow) {
  ...
  const doGeneratePDF = async (filename?: string) => {
    try {
      ...
      const pdfFilename = filename || selectedFile || "Medical Requisition Form";
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                await Share.share({
                  ...
                  text: "Medical Requisition Form PDF generated successfully",
                  url: tempFile.uri,
  ...
  const doGenerateCSV = async (filename?: string) => {
    try {
      ...
      const csvFilename = filename || selectedFile || "Medical Requisition Form";
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                await Share.share({
                  ...
                  text: "Medical Requisition Form data exported as CSV",
                  url: tempFile.uri,
  ...
  const sendEmail = async () => {
    if (isPlatform("hybrid")) {
      try {
        const content = AppGeneral.getCurrentHTMLContent();
        const invoiceIdentifier = invoiceId || selectedFile || "Medical Requisition Form";

        // For iOS, try using Share API with PDF as it's more reliable
        if (isPlatform("ios")) {
          try {
            // Generate PDF for sharing via email on iOS
            setToastMessage("Preparing medical requisition form for email...");
            setShowToast1(true);
              ...
              reader.onloadend = async () => {
                ...
                try {
                  ...
                  await Share.share({
                    ...
                    text: `Please find the attached medical requisition form with ID: ${invoiceIdentifier}`,
                    url: tempFile.uri,
                    dialogTitle: "Share Medical Requisition Form via Email",
                  });
                  ...
                  setToastMessage("Failed to share medical requisition form. Please try again.");
                  setShowToast1(true);
                }
              };
              reader.readAsDataURL(pdfBlob as Blob);
          ...
          await EmailComposer.open({
            ...
            cc: [],
            bcc: [],
            body: `Please find the attached medical requisition form with ID: ${invoiceIdentifier}`,
