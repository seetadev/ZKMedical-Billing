...
import { getFirebaseTemplateFromLocalStorage } from "../firebase/templateService";
...
const Home: React.FC = () => {
  ...
  const handleSaveToCloud = async () => {
    console.log("☁️ handleSaveToCloud: Starting cloud save");

    if (!fileName) {
      setToastMessage("No file selected to save to cloud.");
      setToastColor("warning");
      setShowToast(true);
      return;
    }
    ...
    try {
      ...
      console.log("📄 handleSaveToCloud: Getting current spreadsheet content");
      const content = encodeURIComponent(AppGeneral.getSpreadsheetContent());

      // Get existing metadata from local storage if it exists
      console.log("📂 handleSaveToCloud: Getting existing file metadata");
      let fileData;
      try {
        fileData = await store._getFile(fileName);
      } catch (error) {
        // File might not exist locally, use current data
        console.log(
          "⚠️ handleSaveToCloud: File not found locally, using current data"
        );
      }

      console.log(
        "☁️ handleSaveToCloud: Creating LocalFile object for cloud save"
      );
      const localFile = new LocalFile(
        fileData?.created || new Date().toISOString(),
        new Date().toISOString(),
        content,
        fileName,
        billType,
      ...
      setToastColor("success");
      setShowToast(true);
      console.log("✅ handleSaveToCloud: Cloud save completed successfully");
    } catch (error) {
      console.error("❌ handleSaveToCloud: Error during cloud save", error);
      setToastMessage("Failed to save file to cloud. Please try again.");
      setToastColor("danger");
      setShowToast(true);
    }
  ...
  const initializeApp = async () => {
    ...
    try {
      ...
      let templateData = DATA[templateId];

      if (!templateData) {
        // Check Firebase templates in local storage
        console.log(
          "🔍 initializeApp: Template not found in DATA, checking Firebase templates in local storage"
        );
        const firebaseTemplate =
          getFirebaseTemplateFromLocalStorage(templateId);

        if (firebaseTemplate) {
          console.log(
            "✅ initializeApp: Firebase template found in local storage",
            { templateId }
          );
          templateData = firebaseTemplate;
        } else {
          console.log(
            "❌ initializeApp: Template not found in DATA library or Firebase local storage",
            {
              templateId,
        ...
        >
          <IonContent>
            <IonList>
              ...
              <IonItem button onClick={handleSaveToCloud} disabled={!user}>
                <IonIcon icon={cloudUploadOutline} slot="start" />
                <IonLabel>
                  <h3>Save to Cloud</h3>
                  {!user && <p>Please sign in to save to cloud</p>}
                </IonLabel>
              </IonItem>

              <IonItem>
                <IonIcon icon={toggleOutline} slot="start" />
                <IonLabel>
