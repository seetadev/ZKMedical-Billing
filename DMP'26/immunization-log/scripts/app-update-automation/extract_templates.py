...
html_path = os.path.join(script_dir, 'templates/immunization log/tab-home.html')
...
pattern_tab = re.compile(r'<textarea[^>]*id="sheetdata"[^>]*>(.*?)</textarea>', re.DOTALL | re.IGNORECASE)
pattern_mob = re.compile(r'<textarea[^>]*id="sheetdata1"[^>]*>(.*?)</textarea>', re.DOTALL | re.IGNORECASE)
...
tablet_template = {
    ...
    "msc": json_tab,
    "footers": [
        {"name": "Record 1", "index": 1, "isActive": True},
        {"name": "Record 2", "index": 2, "isActive": True},
        {"name": "Record 3", "index": 3, "isActive": True},
        {"name": "Record 4", "index": 4, "isActive": True}
    ],
    "appMapping": {}
}
