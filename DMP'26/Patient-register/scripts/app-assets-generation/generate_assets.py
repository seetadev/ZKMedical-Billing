...
def find_assets(root_dir):
    ...
    root_image = os.path.join(root_dir, 'image.png')
    if os.path.exists(root_image):
        selected_icon = root_image
    else:
    ...
    if selected_icon:
        print(f"🎯 Selected Source Icon: {os.path.relpath(selected_icon, root_dir)}")
    else:
    ...
    return selected_icon, portrait_pngs[0][0] if portrait_pngs else None
