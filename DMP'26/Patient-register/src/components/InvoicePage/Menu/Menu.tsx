...
const APP_NAME = "Patient Register";
...
const Menu: React.FC<{
  ...
  const doPrint = async () => {
    ...
    if (isPlatform("hybrid") && isPlatform("android")) {
      try {
        ...
        await Printer.print({
          content: htmlContent,
          name: selectedFile || "Patient Register",
      ...
      const printWindow = window.open("", "Print Patient Register");
      if (printWindow) {
  ...
  const doGeneratePDF = async (filename?: string) => {
    try {
      ...
      const pdfFilename = filename || selectedFile || "Patient Register";
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                await Share.share({
                  ...
                  text: "Patient Register PDF generated successfully",
                  url: tempFile.uri,
  ...
  const doGenerateCSV = async (filename?: string) => {
    try {
      ...
      const csvFilename = filename || selectedFile || "Patient Register";
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                await Share.share({
                  ...
                  text: "Patient Register data exported as CSV",
                  url: tempFile.uri,
  ...
  const sendEmail = async () => {
    if (isPlatform("hybrid")) {
      try {
        const content = AppGeneral.getCurrentHTMLContent();
        const invoiceIdentifier = invoiceId || selectedFile || "Patient Register";

        // For iOS, try using Share API with PDF as it's more reliable
        if (isPlatform("ios")) {
          try {
            // Generate PDF for sharing via email on iOS
            setToastMessage("Preparing patient register for email...");
            setShowToast1(true);
              ...
              reader.onloadend = async () => {
                ...
                try {
                  ...
                  await Share.share({
                    ...
                    text: `Please find the attached patient register with ID: ${invoiceIdentifier}`,
                    url: tempFile.uri,
                    dialogTitle: "Share Patient Register via Email",
                  });
                  ...
                  setToastMessage("Failed to share patient register. Please try again.");
                  setShowToast1(true);
                }
              };
              reader.readAsDataURL(pdfBlob as Blob);
          ...
          await EmailComposer.open({
            ...
            cc: [],
            bcc: [],
            body: `Please find the attached patient register with ID: ${invoiceIdentifier}`,
