...
const APP_NAME = "Medical Invoice";
...
const Menu: React.FC<{
  ...
  const doPrint = async () => {
    ...
    if (isPlatform("hybrid") && isPlatform("android")) {
      try {
        ...
        await Printer.print({
          content: htmlContent,
          name: selectedFile || "Medical Invoice",
      ...
      const printWindow = window.open("", "Print Medical Invoice");
      if (printWindow) {
  ...
  const doGeneratePDF = async (filename?: string) => {
    try {
      ...
      const pdfFilename = filename || selectedFile || "Medical Invoice";
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                await Share.share({
                  ...
                  text: "Medical Invoice PDF generated successfully",
                  url: tempFile.uri,
  ...
  const doGenerateCSV = async (filename?: string) => {
    try {
      ...
      const csvFilename = filename || selectedFile || "Medical Invoice";
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                await Share.share({
                  ...
                  text: "Medical Invoice data exported as CSV",
                  url: tempFile.uri,
  ...
  const sendEmail = async () => {
    if (isPlatform("hybrid")) {
      try {
        const content = AppGeneral.getCurrentHTMLContent();
        const invoiceIdentifier = invoiceId || selectedFile || "Medical Invoice";

        // For iOS, try using Share API with PDF as it's more reliable
        if (isPlatform("ios")) {
          try {
            // Generate PDF for sharing via email on iOS
            setToastMessage("Preparing medical invoice for email...");
            setShowToast1(true);
              ...
              reader.onloadend = async () => {
                ...
                try {
                  ...
                  await Share.share({
                    ...
                    text: `Please find the attached medical invoice with ID: ${invoiceIdentifier}`,
                    url: tempFile.uri,
                    dialogTitle: "Share Medical Invoice via Email",
                  });
                  ...
                  setToastMessage("Failed to share medical invoice. Please try again.");
                  setShowToast1(true);
                }
              };
              reader.readAsDataURL(pdfBlob as Blob);
          ...
          await EmailComposer.open({
            ...
            cc: [],
            bcc: [],
            body: `Please find the attached medical invoice with ID: ${invoiceIdentifier}`,
