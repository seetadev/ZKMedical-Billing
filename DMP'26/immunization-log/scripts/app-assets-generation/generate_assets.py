...
def find_assets(root_dir):
    ...
    """
    exclude_dirs = {'node_modules', 'dist', 'build', 'ios', 'android', '.git', '.gradle', '.idea', 'ios_backup'}
            ...
            try:
                with Image.open(file_path) as im:
                    ...
                    if width == height and width >= 48:
                        ...
                        if 'appicon.appiconset' in lower_path:
                            score += 10000
...
def process_branding(root_dir, source_icon_path=None, source_splash_path=None):
    auto_icon, auto_splash = None, None
    if not source_icon_path or not source_splash_path:
        auto_icon, auto_splash = find_assets(root_dir)
        
    source_icon = source_icon_path if source_icon_path else auto_icon
    source_splash = source_splash_path if source_splash_path else auto_splash
    
    if source_icon and not os.path.isabs(source_icon):
        source_icon = os.path.abspath(os.path.join(root_dir, source_icon))
    if source_splash and not os.path.isabs(source_splash):
        source_splash = os.path.abspath(os.path.join(root_dir, source_splash))
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate branding assets for the app.")
    parser.add_argument("--icon", help="Path to the source icon image.")
    parser.add_argument("--splash", help="Path to the source splash image.")
    args = parser.parse_args()
    
    root_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    process_branding(root_dir, source_icon_path=args.icon, source_splash_path=args.splash)
