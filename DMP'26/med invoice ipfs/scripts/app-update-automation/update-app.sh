...
txt = re.sub(
    r'(className="action-btn"[\s\S]+?style=\{\{[\s\S]+?\}\}\s*>\s*\n\s*)([^\n<]+)(\s*\n\s*</IonButton>)',
...
footer_map = data["templates"]["footerTabNames"]
...
for tpl_file in ["mobile.json", "tablet.json"]:
    ...
    if "footers" in tpl:
        for footer in tpl["footers"]:
            idx_str = f"tab{footer['index']}"
            if idx_str in footer_map:
                footer["name"] = footer_map[idx_str]
