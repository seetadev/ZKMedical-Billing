...
.controller('ListCtrl', function($scope ,LocalFiles ,$state, $ionicActionSheet, $timeout) {
   ...
   $scope.delete = function(file){
    ...
    $scope.view = function(file){
            ...
            navigator.notification.confirm("You are opening a new file. Press Yes to continue opening the file. Press No and return back to your previous file",onConfirm, "New File", ['No','Yes']);
            }
            else{
            var bool = LocalFiles.view(file);
            if(bool){
...
.controller('OptionsCtrl', function($scope, $ionicLoading , CloudService , $http, $state, ModalService, $interval, LocalFiles) {
    
    $scope.signature = { show: false};
            
    var devicefind = getDeviceType();
    if(devicefind == "iPad" || devicefind == "default"){
        $scope.signature.show = true;
    }
   ...
   // save and save as starts here
            ...
            request(messages, function(result){
                    ...
                    message.type="POST";
                    message.format = 'json';
                    message.data={action:"savecurrentfile",fname:fname,data:val1,appname:"Quote Invoice"};
...
.controller('ListFilesWebCtrl', function($scope, $timeout , $ionicLoading , $interval, $state, CloudService ,$ionicHistory, $ionicActionSheet){
            ...
            CloudService.checkLogin().then(function(response){
                   ...
                   //logged in
                   var files = new Array();
                   var message ={action:"listdir",appname:"Quote Invoice"};
            ...
            CloudService.checkLogin().then(function(response){
               ...
               var message ={action:"deletefile",appname:"Quote Invoice",fname:file};
            ...
            var message ={action:"getfile",appname:"Quote Invoice",fname:file};
...
var data={action:"logout",appname:"Quote Invoice"};
...
.controller('PurchaseCtrl', function($scope, Items ,CloudService, $timeout ,$ionicLoading, $interval, ModalService, $ionicActionSheet) {
   ...
   //alert(JSON.stringify($scope.items));
    ...
    if(item == "save"){
        ...
        for(i in products){
        if(products[i].Id == "2015qi10share") continue;
    ...
    $scope.purchasePDF = function(id){
        ...
        if(id == "2014qi10Pdf" || id == "2014qi25Pdf" || id == "2014qi50Pdf" || id == "2014qi100Pdf"){
            for(var i=0;i<4;i++){
        ...
        else if(id == "2015qi10share"){
            if(products[4].Purchase == "Yes")
            return;
        }
        else if(id == "2015qiCloud"){
