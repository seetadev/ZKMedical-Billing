  ...
  for (const [key, dev] of Object.entries(devicesToCapture)) {
    ...
    try {
      ...
      const handleSaveAlert = async (alertLocator, fileNameVal) => {
        ...
        await page.waitForTimeout(1000); // Wait for alert transition and hydration to complete
        const input = alertLocator.locator('input.alert-input');
        await input.focus();
        await input.fill(fileNameVal);
        await page.waitForTimeout(1000); // Wait for input state to settle
      ...
      await page.click('ion-tab-button[tab="files"]');
