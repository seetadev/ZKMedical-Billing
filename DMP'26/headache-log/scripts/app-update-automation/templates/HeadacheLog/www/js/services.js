...
.factory('CloudService', function($http){
         ...
         $http.get("http://aspiringapps.com/webapp",{params:{action:"getInapp",appname:"Headache Log"}}).
         success(function(response) {
                 ...
                 var message = {action:"update",appname:"Headache Log"};
                 $http({url:"http://aspiringapps.com/webapp",method:"POST",params:message}).
                 success(function(responses){
                         var results = responses.result;
                         ////alert("sencond:"+result);
                         if(results == "ok"){
                         var productList=[{"Feature": "save","Id": "2016hclCloud","Purchase":"Yes","Consumed":0,"Own":5}];
                 ...
                 var items;
                 if(result.own == 0){
                 items=[{"Feature": "save","Id": "2016hclCloud","Purchase":"No","Consumed":result.consumed,"Own":result.own}];
                 }
                 else{
                 items=[{"Feature": "save","Id": "2016hclCloud","Purchase":"Yes","Consumed":result.consumed,"Own":result.own}];
                 }
         ...
         $http.get("http://aspiringapps.com/restore",{params:{action:"getInapp",appname:"Headache Log"}}).
         success(function(response) {
                 // this callback will be called asynchronously
                 // when the response is available
                 var result = response.result;
...
.factory('Items',function(){
         ...
         desc = "2000 times Email, Print and Save as";
         price = 6.99;
         }
         else{
         continue;
...
.factory('ModalService',function($ionicModal, $timeout, CloudService){
         ...
         var message = {action:"login",uuid:uuid,password:pass,deviceId:device.uuid,appname:"Headache Log"};
         ...
         CloudService.request(message).then(function(response){
                                            ...
                                            var message = {action:"getInapp",appname:"Headache Log"};
         ...
         var message={action:"register",uuid:uuid,password:pass,appname:"Headache Log"};
...
.factory('MoreAppService', function(){
         ...
         appbundles = [{name: 'Business Suite', desc: 'Get a comprehensive view of your business',
                       ...
                       {name: 'Headache Log Suite', desc: 'Managing your finances and do financial book keeping',
