  ...
  for (const [key, dev] of Object.entries(devicesToCapture)) {
    ...
    try {
      ...
      const handleSaveAlert = async (alertLocator, fileNameVal) => {
        ...
        // Wait up to 3 seconds for the dialog to disappear or detach.
        // If the page reloads/navigates, the element will detach, which is ignored.
        try {
          await alertLocator.waitFor({ state: 'detached', timeout: 3000 });
        } catch (e) {
          // Safe to ignore if navigation has already begun
        }
      };
